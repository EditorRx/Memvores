...
// ===== Findy AI Assistant (Local-First + Optional API) =====
...
// ===== Local savage replies (always work) =====
const LOCAL_REPLIES = {
  greeting: [
    "Yo. I'm Findy. Drop what you're looking for and I'll dig it up... or roast you for asking something dumb.",
    "Findy online. Tell me what chaos you need.",
    "What's good? Looking for clips or just here to waste my time?"
  ],
  noResults: [
    "Bruh... even I can't find that. Try different keywords.",
    "Nothing in the vault matches that. You sure you spelled it right?",
    "I searched the whole archive. Zero. Try again with better words."
  ],
  found: [
    "Found these. Don't say I never give you anything:",
    "Here. These slap harder than your last idea:",
    "Got 'em. You're welcome."
  ],
  chat: [
    "You talking or you actually need something?",
    "Interesting. Still waiting for a real request though.",
    "Cool story. Now tell me what clip or audio you want.",
    "I'm a meme hunter, not your therapist. What do you need?",
    "Noted. Now drop some keywords if you want media.",
    "Savage mode is always on. What are you looking for?"
  ],
  error: [
    "Something glitched. Try again.",
    "Even I have bad days. Hit me again."
  ]
};
...
function openFindy() {
  ...
  if (findyMessages && findyMessages.children.length === 0) {
    addFindyMessage('ai', pick(LOCAL_REPLIES.greeting));
  }
...
function findySearchPosts(query, category = null, limit = 4) {
  ...
  if (!q && !category) return all.slice(0, limit);

  const filtered = all.filter((post) => {
    const caption = (post.caption || '').toLowerCase();
    const type = (post.type || '').toLowerCase();
    const cat = (post.category || '').toLowerCase();
    const matchesQuery = !q || caption.includes(q) || type.includes(q) || cat.includes(q);
...
function looksLikeMediaRequest(text) {
  ...
  const words = [
    'clip', 'clips', 'video', 'audio', 'sound', 'template', 'templates',
    'tutorial', 'tutorials', 'meme', 'find', 'search', 'dikhao', 'do',
    'bhej', 'chahiye', 'need', 'want', 'send', 'show', 'kahan', 'hai',
    'milega', 'de', 'la', 'lao', 'nikal', 'nikal de'
  ];
...
function detectCategory(text) {
  ...
  if (t.includes('template') || t.includes('edit')) return 'templates';
  if (t.includes('tutorial') || t.includes('how to') || t.includes('sikhao')) return 'tutorials';
  if (t.includes('clip') || t.includes('video') || t.includes('meme')) return 'clips';
  return null;
}

async function tryAPIChat(userText) {
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 6000);

    const res = await fetch(FINDY_API_URL, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are Findy from Memevores. Personality: sarcastic, savage (9/10), short, punchy, meme-fluent. Roast lightly. Keep answers under 2 sentences. Match user's language (Hindi-English mix ok).`
          },
          ...findyHistory.slice(-6),
...
async function sendFindyMessage() {
  ...
  try {
    // ===== MEDIA PATH (always works offline) =====
    if (looksLikeMediaRequest(text)) {
      const category = detectCategory(text);
      const results = findySearchPosts(text, category, 4);

      typing.remove();

      if (results.length === 0) {
        const msg = pick(LOCAL_REPLIES.noResults);
        addFindyMessage('ai', msg);
        findyHistory.push({ role: 'assistant', content: msg });
      } else {
        const msg = pick(LOCAL_REPLIES.found);
        addFindyMessage('ai', msg);
    ...
    // ===== NORMAL CHAT =====
    const apiReply = await tryAPIChat(text);

    typing.remove();

    if (apiReply) {
      addFindyMessage('ai', apiReply);
      findyHistory.push({ role: 'assistant', content: apiReply });
    } else {
      const msg = pick(LOCAL_REPLIES.chat);
      addFindyMessage('ai', msg);
      findyHistory.push({ role: 'assistant', content: msg });
    }
  } catch (err) {
    typing.remove();
    const msg = pick(LOCAL_REPLIES.error);
    addFindyMessage('ai', msg);
